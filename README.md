...
![版本](https://img.shields.io/badge/版本-v4.5.0-0a84ff?style=flat-square)
...
| 🚀 **开发者功能** | FPS 监控（**自适应屏幕刷新率上限**）、基础/高级内存自动回收、系统日志导出/清空、沙箱控制、DOMGuard 界面防护 |
| 📱 **移动优化** | 触摸手势、长按菜单、滑动翻页、震动反馈、PWA 支持、安全区域适配、双击顶部返回、**拖拽安装** |
| 🌐 **离线能力** | PWA 离线缓存、离线提示、断网可用 |
| 🛡️ **安全防护** | DOMGuard 界面防护系统（**防移除/防隐藏/防 head 注入/防主题篡改/核心函数防篡改/背景防篡改**）、紧急退出机制（Esc/F12）、应用沙箱隔离（**关闭后仍保留最小防护**）、密码保护（AES-GCM 加密存储）、**应用防伪装校验**、**高危操作黑名单拦截**、**iframe 彻底重建机制** |
| 📢 **通知系统** | Toast 通知（限流队列）、通知中心（分类过滤、持久化）、通知声音；玻璃主题下通知面板支持毛玻璃效果 |
| 👤 **用户系统** | 自定义用户名、Emoji 头像选择（**支持本地上传图片头像**）、系统密码管理（AES-GCM 加密存储） |
| 🔋 **硬件集成** | 电池状态实时显示、摄像头/麦克风/震动硬件检测（安全中心） |
...
| **iframe + sandbox** | 应用隔离运行、安全执行（支持关闭沙箱的开发者选项，**关闭后仍保留最小防护**） |
| **postMessage** | 系统与应用双向通信（限流：每秒最多 30 条，防消息洪水；**数据体积限制：防超大payload滥用**） |
| **Service Worker** | 离线缓存、PWA 支持 |
| **MutationObserver** | DOMGuard 界面防护系统（防移除、防隐藏、防 head 注入、防主题篡改、核心函数防篡改、背景防篡改） |
| **Web Crypto API** | 系统密码 AES-GCM 加密存储 |
| **Battery Status API** | 电量实时显示 |
| **Vibration API** | 触摸反馈 |
| **File System Access / Drag & Drop** | 批量/文件夹/**拖拽**应用安装 |
| **Intl.DateTimeFormat** | 高性能日期时间格式化（缓存复用） |
| **requestAnimationFrame** | 渲染合帧优化、屏幕刷新率实测 |
...
> ⚠️ **安全限制**：以下高危操作会被系统强制拦截：`eval`、`execScript`、`setPassword`、`disableSandbox`、`toggleSandbox`、`grantPermission`、`elevate`、`navigateTop`、`setAsSystem` 等。
...
- **解锁功能**：
  ...
  - 高级内存优化（自动清理缓存、旧数据）
  - 沙箱开关控制（**危险操作，需密码+知情确认+输入确认短语三重验证**）
  - 系统日志导出/清空
  - 强制刷新所有应用、修复磁贴布局
...
- **特性**：内置于系统代码，**无法卸载、无法隐藏、无法篡改、无法重命名**
...
| **导入备份** | 支持「合并」（保留现有数据）和「覆盖」两种模式；**系统核心应用不通过备份导入** |
...
| **玻璃质感** | 毛玻璃效果、大圆角、丝滑动画、阴影层次；**通知中心与 Toast 支持 backdrop-filter 实时模糊**；**性能优化：预模糊壁纸+最小化实时模糊层** |
...
```
HtmlOS/
├── index.html              # 单文件完整系统（v4.5.0）
├── README.md               # 项目说明
└── .github/                # GitHub 配置
```

> **单文件设计**：所有功能（UI、逻辑、样式、PWA、IndexedDB 封装、DOMGuard、安全中心、密码加密、拖拽安装、防篡改机制）集成在一个 HTML 文件中，无需构建工具，直接部署。
...
| 机制 | 说明 |
|------|------|
| **沙箱隔离** | iframe `sandbox` 属性隔离应用，**彻底移除 `allow-same-origin`**，切断应用越权访问父页面的路径 |
| **最小防护** | 即使关闭沙箱，仍保留 `sandbox` 属性（至少 `allow-scripts allow-forms allow-popups allow-modals allow-downloads allow-popups-to-escape-sandbox`），绝不完全移除 |
| **DOMGuard** | 6 重防护：①关键元素防移除 ②关键元素防隐藏 ③head 注入拦截 ④主题类防篡改 ⑤核心函数防篡改 ⑥背景 CSS 变量防篡改 |
| **消息鉴权** | postMessage 白名单机制 + 高危操作黑名单拦截 + 每秒 30 条限流 + 单条 8000 字符数据体积限制 |
| **密码加密** | Web Crypto API AES-GCM 加密存储，PBKDF2 派生密钥，失败安全回退 |
| **应用防伪装** | ①来源字段强制归一化（仅安全中心可为 `system`）②禁止用户应用使用系统保留名称（如「安全中心」） |
| **iframe 重建** | 应用关闭时彻底重建 iframe（清空 srcdoc、移除旧节点、创建新节点），消除残留风险 |
| **紧急退出** | Esc / F12 快捷键强制返回桌面（绕过关闭确认） |
